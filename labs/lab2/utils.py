...
def plot_vectors():
    ...
    # Plot v1 and label
    plt.quiver(
        0, 0, v1[0], v1[1], angles="xy", scale_units="xy", scale=1, color="#fb5c6b"
    )
    plt.text(
        v1[0] + 0.1,
        v1[1],
        f"v1: {tuple(int(x) for x in v1)}",
        fontsize=9,
        color="#191c24",
    )
    ...
    # Plot v2 and label
    plt.quiver(
        0, 0, v2[0], v2[1], angles="xy", scale_units="xy", scale=1, color="#fb5c6b"
    )
    plt.text(
        v2[0] + 0.1,
        v2[1],
        f"v2: {tuple(int(x) for x in v2)}",
        fontsize=9,
        color="#191c24",
    )
    ...
    for i, av in enumerate(array_v):
        plt.quiver(
            0, 0, av[0], av[1], angles="xy", scale_units="xy", scale=1, color="#08b4cc"
        )
        plt.text(
            av[0] + 0.1,
            av[1],
            f" av[{i}]: {tuple(int(x) for x in av)}",
            fontsize=9,
            color="#191c24",
        )
    ...
    for i in range(len(array_v)):
        plt.text(
            0.5,
            y_start - (2 * i * step),
            f"Cos(v1, av[{i}]) = {cos_sim_v1_array_v[i]:.4f}",
            color="#191c24",
        )
        plt.text(
            0.5,
            y_start - ((2 * i + 1) * step),
            f"Dist(v1, av[{i}]) = {euc_dist_v1_array_v[i]:.4f}",
            color="#191c24",
        )
        plt.text(
            3.5,
            y_start - (2 * i * step),
            f"Cos(v2, av[{i}]) = {cos_sim_v2_array_v[i]:.4f}",
            color="#191c24",
        )
        plt.text(
            3.5,
            y_start - ((2 * i + 1) * step),
            f"Dist(v2, av[{i}]) = {euc_dist_v2_array_v[i]:.4f}",
            color="#191c24",
        )
...
def display_widget(model):
    ...
    def plot_embeddings():
        with output_plot:
            ...
            colormap = ListedColormap(
                (pastel_colors * (color_count // len(pastel_colors) + 1))[:color_count]
            )

            texts = []
            for color, (label, (x, y)) in zip(colormap.colors, zip(sentences, embeddings_2d)):  # type: ignore
    ...
    add_button = Button(
        description="Add Word or Sentence!", layout=Layout(width="200px")
    )
    ...
    display(
        VBox(
            [output_plot], layout=Layout(align_items="center", justify_content="center")
        )
    )
    display(HTML("</div>"))
...
def generate_with_single_input(
    ...
    **kwargs,
    ...
    if (not together_api_key) and ("TOGETHER_API_KEY" not in os.environ):
        ...
        resp: ChatCompletionResponse = client.chat.completions.create(**payload)  # type: ignore
