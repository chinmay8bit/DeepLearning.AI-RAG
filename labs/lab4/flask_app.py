    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return (
                    jsonify({"error": f"Could not parse request body: {e_inner}"}),
                    400,
                )
        ...
        if not isinstance(text, dict) or "query" not in text or "documents" not in text:
            print(
                f"Invalid input format. Expected dict with 'query' and 'documents'. Got: {text}"
            )
            return (
                jsonify(
                    {
            ...
            reranked_results.append(
                {
                    ...
                    "score": float(
                        score
                    ),  # Use "score" as the key, ensuring float type
                }
            )

        return jsonify(
            {"scores": reranked_results}
        )  # Top-level key is "scores" (plural)
