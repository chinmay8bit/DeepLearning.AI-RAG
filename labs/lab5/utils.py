...
def kill_processes_on_ports(
    ...
    *,
    ...
    if alive and force:
        for p in alive:
            try:
                ...
                results["errors"].append(
                    {"pid": p.pid, "error": f"Access denied on kill: {e}"}
                )
            except Exception as e:
        ...
        for p in alive2:
            results["errors"].append(
                {"pid": p.pid, "error": "still alive after kill()"}
            )
...
EMBEDDING_MODELS = {
    "BAAI/bge-base-en-v1.5": SentenceTransformer("BAAI/bge-base-en-v1.5"),
}
...
def generate_embedding(
    ...
    if model in EMBEDDING_MODELS:
        return EMBEDDING_MODELS[model].encode(prompt).tolist()
...
def display_widget(
    ...
    def on_button_click(b):
        ...
        for output, func, query, retriever, use_rag, *extra_info in retrievals:
            ...
            response = func(
                ...
                top_k=top_k,
                use_rag=use_rag,
                retrieve_function=retriever,
                use_rerank=use_rerank,
                rerank_property=rr_property,
            )
            with output:
    ...
    query_label = widgets.Label(
        value="Query:", layout=widgets.Layout(width="10%", align="center")
    )
    query_box = widgets.HBox(
        [query_label, query_input], layout=widgets.Layout(align_items="center")
    ...
    hbox_outputs1 = widgets.HBox(
        [vbox1, vbox4, vbox2], layout={"justify_content": "space-between"}
    )
    hbox_outputs2 = widgets.HBox(
        [vbox3, vbox5], layout={"justify_content": "space-between"}
    )
